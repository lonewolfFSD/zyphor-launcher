...
function getWeatherMeta(code, isNight = false) {
  ...
  if (code === 1) return isNight ? { icon: faCloudMoon, color: '#93c5fd', label: 'Mainly clear' } : { icon: faCloudSun, color: '#fcd34d', label: 'Mainly clear' };
...
const BLANK = new Set(['Classified', 'Unknown', '', null, undefined]);

async function fetchWeatherForLoc(loc, signal) {
  if (BLANK.has(loc)) {
      ...
      navigator.geolocation.getCurrentPosition(
        async ({ coords: { latitude, longitude } }) => {
          try {
            ...
            resolve(d?.current ? { temp: Math.round(d.current.temperature_2m), code: d.current.weather_code, humidity: d.current.relative_humidity_2m, place: 'Nearby' } : null);
          } catch { resolve(null); }
        },
        () => resolve(null),
  ...
  const geoRes = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(loc)}&count=1`, { signal });
...
function BatteryIcon({ level, charging, color }) {
  ...
  const fillW = Math.max(0, Math.min(10, Math.round(level * 10)));
  const fillColor = charging ? '#4ade80' : pct <= 15 ? '#f87171' : pct <= 30 ? '#fb923c' : color;
  return (
    <svg width="18" height="10" viewBox="0 0 18 10" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ flexShrink: 0 }}>
      <rect x="0.5" y="0.5" width="15" height="9" rx="2" stroke={fillColor} strokeOpacity="0.6" strokeWidth="1" />
      <rect x="16" y="3" width="2" height="4" rx="1" fill={fillColor} fillOpacity="0.5" />
      <rect x="2" y="2" width={fillW} height="6" rx="1" fill={fillColor} />
      {charging && <text x="4.5" y="8.5" fontSize="7" fill="#fff" fontWeight="bold" style={{ userSelect: 'none' }}>⚡</text>}
    </svg>
  );
}
...
export default function TitleBar({ profile }) {
  ...
  useEffect(() => {
    ...
    const onLevel = (e) => update(e.target);
    const onCharge = (e) => update(e.target);
    ...
    return () => {
      if (bat) {
        ...
        bat.removeEventListener('chargingchange', onCharge);
      }
    };
  }, []);

  // Resolve location first, THEN fetch weather — fixes the race where weather
    ...
    async function run() {
      ...
      if (!BLANK.has(propLoc)) {
        ...
        try {
          ...
          if (!ac.signal.aborted && snap.exists()) {
            const fl = snap.data()?.location ?? null;
            loc = BLANK.has(fl) ? null : fl;
          }
        } catch { /* fall through */ }
      }
      ...
      try {
        const result = await fetchWeatherForLoc(loc, ac.signal);
        if (!ac.signal.aborted) setWeather(result);
      } catch {
  ...
  async function handleCheckUpdates() {
    ...
    try { await window.launcherAPI?.checkForUpdates?.(); }
    catch (e) { console.warn('Update check failed', e); }
    finally { setTimeout(() => setUpdateChecking(false), 1800); }
  }
  ...
  const menuItems = [
    { icon: faBug, label: 'Report a bug', sub: 'Open GitHub issue', action: () => openExternal('https://github.com/zyphorstudios/launcher/issues/new?template=bug_report.md') },
    { icon: faCircleQuestion, label: 'Help & support', sub: 'Docs and FAQs', action: () => openExternal('https://zyphorstudios.com/support') },
    { icon: faDiscord, label: 'Join Discord', sub: 'Community server', action: () => openExternal('https://discord.gg/zyphor') },
    { divider: true },
    { icon: faRotate, label: updateChecking ? t('titleBar.updating', {}, 'Checking…') : t('settings.about.checkForUpdates', {}, 'Check for updates'), sub: `Current: v${APP_VERSION}`, action: handleCheckUpdates },
    { icon: faCircleInfo, label: t('settings.about.title', {}, 'About Zyphor Launcher'), sub: 'Version info', action: () => window.launcherAPI?.openAbout?.() ?? openExternal('https://zyphorstudios.com') },
  ];

  return (
    <div
      className="relative flex h-9 shrink-0 items-center justify-between border-b"
      style={{ WebkitAppRegion: 'drag', backgroundColor: `${theme.surface}e0`, borderColor: theme.border, color: theme.text }}
    >
      <motion.div
      ...
      {/* Left — wordmark + menu */}
      <div className="flex items-center gap-0 pl-3" style={{ WebkitAppRegion: 'no-drag' }}>
        {isVip && (
          <motion.span animate={{ opacity: [0.5, 1, 0.5] }} transition={{ repeat: Infinity, duration: 2 }} className="mr-2 text-[10px] select-none" style={{ color: vipGold }}>
            ✦
          </motion.span>
        )}
        <button
      ...
      >
        ...
        <div className="flex items-center gap-1.5 min-w-[90px]">
          {weatherLoading ? (
            ...
            <>
              <FontAwesomeIcon icon={wMeta.icon} className="text-[13px]" style={{ color: wMeta.color, filter: `drop-shadow(0 0 4px ${wMeta.color}66)` }} title={wMeta.label} />
              <span className="font-mono text-[12px] font-semibold tabular-nums" style={{ color: theme.text }}>{weather.temp}°</span>
        ...
        {/* Battery — hidden on desktops where getBattery returns no battery */}
        {battery !== null && (
          <>
            <span className="h-3.5 w-px" style={{ backgroundColor: theme.border }} />
      ...
      <div className="flex items-center gap-0.5 pr-2" style={{ WebkitAppRegion: 'no-drag' }}>
